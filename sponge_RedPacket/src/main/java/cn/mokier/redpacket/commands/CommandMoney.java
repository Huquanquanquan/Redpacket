...
public class CommandMoney implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (src instanceof Player) {
            ...
            //如果总额没有达到最低要求
            int mix = (int) Config.getValue("mix");
            if(total < mix) {
                Chat.sendLangMessage(src, "totalMix", false);
                return CommandResult.success();
            }

            //扣取金钱
            Optional<Account> uOpt = RedPacket.economyService.getOrCreateAccount(player.getIdentifier());
            Account acc = uOpt.get();
            BigDecimal requiredAmount = BigDecimal.valueOf(total);
            EventContext eventContext = EventContext.builder().add(EventContextKeys.PLUGIN, RedPacket.pluginContainer).build();

            TransactionResult result = acc.withdraw(RedPacket.economyService.getDefaultCurrency(), requiredAmount, Cause.of(eventContext, RedPacket.pluginContainer));
            if (result.getResult() == ResultType.FAILED || result.getResult() == ResultType.ACCOUNT_NO_FUNDS) {
                Chat.sendLangMessage(player, "notEnoughMoney", false);
                return CommandResult.success();
            }
            ...
            //新建红包
            String packetName = PacketManager.create(player, total, split, false, message.contains("随机"));
